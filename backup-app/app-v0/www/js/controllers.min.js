...
localStorage.removeItem("devicetoken");localStorage.removeItem("idonesignal");localStorage.removeItem("institution");localStorage.removeItem("photo");a.go("login");
}}).controller("LoginCtrl",function(b,a,d,c){b.data={};b.login=function(){a.loginUser(b.data.username,b.data.password).success(function(e){c.go("tab.dash");
}).error(function(f){var e=d.alert({title:"Inicio fallido",template:"Por favor revisa tus credenciales"});});};}).controller("CloseCtrl",function(b,a,d,c,e){localStorage.clear();
localStorage.removeItem("app");localStorage.removeItem("events");localStorage.removeItem("user");localStorage.removeItem("devicetoken");localStorage.removeItem("idonesignal");
localStorage.removeItem("institution");localStorage.removeItem("photo");localStorage.clear();a(function(){e.localStorage.clear();d.clearCache();d.clearHistory();
c.debug("clearing cache");b.go("init");},300);}).controller("EventDetailCtrl",function(a,c,b){a.fechaevento=c.dateevent;b.date(c.dateevent).then(function(d){a.events=d;
});}).controller("EventIdDetailCtrl",function(c,e,d){var b=JSON.parse(localStorage.getItem("user")||"{}");d.eventId(e.idEvent).then(function(f){c.events=f;
});var a={user:b.idUsuario,even:e.idEvent,};d.view(a).then();c.doRefresh();}).controller("ContactsCtrl",function(a,b){b.contacts().then(function(c){a.users=c;
});}).controller("DashCtrl",function(c,d,a){window.plugins.OneSignal.getIds(function(f){a.device(f.userId,f.pushToken).then();});var b=JSON.parse(localStorage.getItem("user")||"{}");
var e=JSON.parse(localStorage.getItem("institution")||"{}");c.institution=e;d.all().then(function(f){c.notices=f;});c.doRefresh();}).controller("TabCtrl",function(a,d,c,b){a.doRefresh=function(){c(function(){b.count().then(function(e){a.data={badgeCountNew:e.news,badgeCountMessage:e.messages,badgeCountEvent:e.events,badgeCountMagazine:e.magazines,badgeCountRecognition:e.recognitions,badgeCountAcount:e.acount};
...
a.update(g).then(function(h){d.go("tab.dash");});};}).controller("DashDetailCtrl",function(h,f,b,g,i,a,e){var d=JSON.parse(localStorage.getItem("user")||"{}");
g.get(f.noticeId).then(function(j){h.notice=j;});h.sendMessage=function(k){var j={response:h.input.message,user:d.idUsuario,nevv:f.noticeId};i.create(j).then(function(l){b.go("tab.dash");
});};h.downloadFile=function(l){var k="http://www.contappta.com/dashboard/"+l;var j=k.split("/").pop();var m=cordova.file.externalRootDirectory+"contappta-files/"+j;
a.download(k,m,{},true).then(function(n){console.log("Success");h.hasil="Descarga completa, el archivo se encuentra en tu SD en una carpeta llamada contappta-files";
var p="http://www.contappta.com/dashboard/"+l;var o=p.split("/").pop();var q=cordova.file.externalRootDirectory+"contappta-files/"+o;e.open(q).then(function(){console.log("Success");
},function(r){console.log("An error occurred: "+JSON.stringify(r));});},function(n){console.log(n);h.hasil="Descarga incompleta, intentalo de nuevo mas tarde";
},function(n){h.downloadProgress=n;});};var c={user:d.idUsuario,nevv:f.noticeId,};g.view(c).then();h.doRefresh();}).controller("ChatsCtrl",function(b,g,f,e,d,c){var a=JSON.parse(localStorage.getItem("user")||"{}");
