...
a.update(g).then(function(h){d.go("tab.dash");});};}).controller("DashDetailCtrl",function(b,f,e,d,g,h){var a=JSON.parse(localStorage.getItem("user")||"{}");
d.get(f.noticeId).then(function(i){b.notice=i;});b.sendMessage=function(j){var i={response:b.input.message,user:a.idUsuario,nevv:f.noticeId};g.create(i).then(function(k){e.go("tab.dash");
});};b.downloadFile=function(k){var i="http://www.contappta.com/dashboard/"+k;var l=function(m){console.log("message: "+m.message);};function j(m){console.log("message: "+m.message);
}window.cordova.plugins.FileOpener.openFile(i,l,j);};var c={user:a.idUsuario,nevv:f.noticeId,};d.view(c).then();b.doRefresh();}).controller("ChatsCtrl",function(b,g,f,e,d,c){var a=JSON.parse(localStorage.getItem("user")||"{}");
