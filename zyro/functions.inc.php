...
function parse_uri() {	
	...
	if (!count($ru)) {
		foreach ($pages as $idx => $pi) {
			if ($home_page == $pi['id']) return array($idx, $def_lang, array());
		}
		return array($home_page, $def_lang, array());
	}
	...
	if ($ru[0] == 'news') {
		...
		array_shift($ru);
		return array($pageIdx, $def_lang, $ru);
	}
	else if ($ru[0] == 'blog') {
		$pageIdx = getPageIndexById(isset($ru[1]) ? intval($ru[1]) : null);
		$show_comments = true;
	...
	foreach ($pages as $idx => $pi) {
		if ($ru_ == $pi['id']) return array($idx, $lang, $ru);
	}
	if (!$ru_) {
		foreach ($pages as $idx => $pi) {
			if ($home_page == $pi['id']) return array($idx, $lang, $ru);
		}
		return array($home_page, $lang, $ru);
	}
	
	foreach ($pages as $idx => $pi) {
		if (is_array($pi['alias'])) {
			if ($lang && isset($pi['alias'][$lang]) && $ru_ == $pi['alias'][$lang]) {
				return array($idx, $lang, $ru);
			}
